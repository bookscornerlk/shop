...
function fetchSearchResults(q) {
    ...
    fetch(`${(window.AK && window.AK.base) || ''}/api/search-products.php?q=${encodeURIComponent(q)}`)
        .then(r => r.json())
        .then(items => {
            if (!items.length) { results.classList.remove('open'); return; }
            results.innerHTML = items.map(p => `
                <a href="${p.url}" class="search-result-item">
                    <img src="${p.image}" alt="${p.name_en}" class="search-result-img" loading="lazy">
                    <div>
                        <div class="search-result-name">${p.name_en}</div>
                        <div class="search-result-price">${p.price_fmt}</div>
                    </div>
                </a>
            `).join('');
